...
# Global dictionary to store the latest computed results for each device tag
latest_results = {}  # Now stores multiple device routes
active_exits = {
    "Entrance": [],
    "Balcony1": [],
    "Balcony2": []
...
def find_nearest_available_exit(user_node):
    ...
    for exit, _ in sorted_exits:
        if len(active_exits[exit]) < EXIT_CAPACITY[exit]:  # Check if the exit has capacity
            return exit
...
#         logging.info(f"\nReceived WiFi devices: {wifi_devices}")
#         logging.info(f"Device Tag: {device_tag}")
        
#         # Process devices with weights
#         devices, distances, weights = process_devices(wifi_devices)
        
#         if len(devices) < 2:
#             return jsonify({
#                 'status': 'failure',
#                 'message': 'Need at least 2 known routers'
#             }), 400
        
#         # Calculate position with weighted triangulation
#         estimated_coord = triangulate_position(devices, distances, weights)
#         if not estimated_coord:
#             return jsonify({
#                 'status': 'failure',
#                 'message': 'Triangulation failed'
#             }), 400
...
#         # Calculate path to entrance
#         path, total_distance = a_star(graph, user_node, "Entrance", set())
...
def process_wifi_data():
    ...
    try:
        ...
        if path:
            ...
            latest_results[device_tag] = result
            active_exits[assigned_exit].append(device_tag) # Mark the exit as occupied
